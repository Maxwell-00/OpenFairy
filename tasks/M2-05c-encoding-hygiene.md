...
- In `packages/kernel/src/governance.ts`: rewrite the four OTP regexes so the anchor alternation contains exactly the semantic alternatives (`otp`, `one[-\s]?time`, `verification`, `verify`, `code`, simplified yanzhengma as `\u9a8c\u8bc1\u7801`, traditional as `\u9a57\u8b49\u78bc` (codepoints given in ASCII on purpose — do not retype CJK from this brief)) — the mojibake fourth alternative is gone. Convert the health/finance/legal category regexes' CJK alternatives to `\uXXXX` escapes with a trailing `// zh: …` comment naming the terms in a comment (comments are not patch anchors for code hunks and keep the file greppable).
- In `packages/kernel/src/persona.ts`: convert `thanksPattern`, `distressPattern`, and the three CJK banned-corpus regexes to `\uXXXX` escapes, same comment convention.
