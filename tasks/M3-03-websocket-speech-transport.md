...
- No external network.
- No TLS/auth/client productization in this slice.
...
- Clean shutdown with no dangling timers/processes.
- No repo-tree writes.
- If adding a dependency, it must be a generic WebSocket dependency, not a vendor audio SDK, and must have no native build approval requirement unless explicitly documented and committed.
...
- External hosts disabled by default.
