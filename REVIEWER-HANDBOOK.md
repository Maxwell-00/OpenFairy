...
M0–M1 closed (text spine). M2 in progress: M2-01 through M2-05 closed (M2-05 countersigned with the mojibake correction — see landmine §3.5). M2-06b acceptance-debt hardening (closed — discharged the M2-02/03/04 retro-audit register D1–D10; `tasks/M2-02-04-retro-audit.md` + `tasks/M2-06b-review.md`). M2-05b affect cache/negative-feedback fix (closed — bucket-only prefix line, `user-negative-feedback` appraisal; found via owner real-provider smoke test, which remains the irreplaceable check class). M2-05c encoding hygiene (closed at `89e335d` — mojibake gone, runtime CJK regex anchors `\uXXXX`-escaped, CI encoding guard in `pnpm lint`; bonus fix: CJK OTP anchors no longer wrapped in inert `\b`, so space-separated Chinese OTP is now actually detected — disclosed by Codex, endorsed in the countersignature). M2-06 perception service (closed at `ceb2a74` — artifact pipeline, `vision.describe`/`vision.ocr`, `perception.quarantine-v0`, OCR secret escalation + routing, all countersigned via opus cross-check, 12/12; real vision/OCR provider + `perception.vision` role wiring remain explicitly-unclaimed future work, and that future slice's owner checks must be real-provider runs). M2-07 context ladder L4/L5 (closed at `dd3b0a6` — compactor-call clearance, quarantine no-laundering, and `session.compacted {range, summary_ref}` all countersigned via opus cross-check; **two-test carry-in for the exit slice's Deliverable 0:** (1) E2E for the terminal no-cleared-summarizer fail-closed skip, (2) integration test driving an invalid-compactor-output turn to completion on the original context path). M2-08 Chronicle v1 + dream-cycle v0 (closed at `5f3ef12` — countersigned via opus cross-check 12/12; both M2-07 carry-in tests landed with it, so that register is discharged; consolidation is deterministic/no-model, canary + contradiction benchmarks visibly deferred; note the synthetic session dirs consolidation creates for its artifact events). **Next and final M2 slice: M2-09 exit consolidation** — no new features; scope: verify all twelve named suites visible/green, assemble M2 exit evidence against ROADMAP exit criteria (S2/S4/S7, leakage/label zero-tolerance; persona ≥ 90% and memory canary need explicit defer-or-satisfy decisions since their judge/benchmark versions are deferred), sqlite-vec vs LanceDB decision gate, produce `tasks/M2-exit-review.md`, state M3 preconditions, refresh this handbook. Docs passes through M2-08 are already applied — do not re-list them in the M2-09 brief. Remaining M2 slices after that: context L4/L5 compaction, Chronicle/dream-cycle, M2 exit consolidation (S7 screenshot flow lands with M2-06). Parallel Cowork conversations exist — check their outputs in `tasks/` rather than assuming this file knows everything.
