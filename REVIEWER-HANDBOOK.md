...
M0–M1 closed (text spine). M2 in progress: M2-01 through M2-05 closed (M2-05 countersigned with the mojibake correction — see landmine §3.5). M2-06b acceptance-debt hardening (closed — discharged the M2-02/03/04 retro-audit register D1–D10; `tasks/M2-02-04-retro-audit.md` + `tasks/M2-06b-review.md`). M2-05b affect cache/negative-feedback fix (closed — bucket-only prefix line, `user-negative-feedback` appraisal; found via owner real-provider smoke test, which remains the irreplaceable check class). **Next: M2-05c encoding hygiene** (tiny — mojibake removal now owned here, `\uXXXX` conversion of runtime CJK regex anchors, CI encoding guard; born from repeated patch-context encoding collisions during M2-05b/06b), **then M2-06 perception service** (brief gated 2026-07-06, amended: no longer carries the mojibake cleanup; `perception.quarantine-v0`; must rebase on M2-05b/05c). Remaining M2 slices after that: context L4/L5 compaction, Chronicle/dream-cycle, M2 exit consolidation (S7 screenshot flow lands with M2-06). Parallel Cowork conversations exist — check their outputs in `tasks/` rather than assuming this file knows everything.
