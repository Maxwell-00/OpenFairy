...
Every assembled prompt logs a **context manifest** event: zone sizes, cache-prefix hash, reduction stages applied, tokens by provenance. The replay debugger renders it as a stacked bar over time — context bugs become visible instead of vibes. Ledger correlates cache-hit rates with cost per provider.
