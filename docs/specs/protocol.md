...
| Affect | `affect.updated` | With cause summary. *Payload reconciled pre-M2-05 (additive): `valence`/`arousal` ∈ -1..1, `stance` enum gains `dry` (persona-affect baseline), `cause` required, optional `energy` (`low\|medium\|high`) + `updated_at`* |
