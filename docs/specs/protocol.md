...
**Transport-level frames vs. envelopes (normative shapes since M1-04):** the gateway→client stream is canonical envelopes, plus exactly two **transport frame** kinds for op-level responses that are not session facts — `{kind: "ack", op, ...}` (op accepted, nothing to log) and `{kind: "op-error", op, message, ...}` (malformed op, unknown sid, bad `request_id` — transport mistakes, not session facts). Every client op yields a deterministic response: session facts as envelopes, non-facts as frames; silent drops are forbidden. Frames are schema'd + fixture-tested in `packages/protocol/frames/` — deliberately **outside** the event registry (they are never appended to session logs). Session-scoped errors remain `error` envelopes. *Since M3-02 a third frame family exists: the gateway⇄speech-worker duplex control frames (voice-pipeline §2 / M3-02 status note), validated + fixture-tested in `packages/voice/fixtures/` — equally outside the event registry, never appended to session logs; binary audio frames are in-memory only, and the replay surface remains canonical `speech.*` events exclusively. M3-03 carries this family over a real loopback WebSocket (text = JSON control frames, binary = audio envelope) without changing the rule: WebSocket messages are transport frames, never events, never JSONL; the canonical voice replay surface is still exactly the four registered `speech.*` types. M3-04 adds a third encoding of the same worker plane: supervised-child stdio NDJSON (`fairy.speech-worker.v0`) — wire request ids, `hello`/`ready`/`bye`, `tts.done`, and cancellation receipts are transport facts only, never runtime events; worker failure surfaces via the existing `progress.update` with stage `voice.worker.failed`, and the canonical replay surface remains exactly `speech.asr.partial` / `speech.asr.final` / `speech.tts.chunk` / `speech.mark`. M3-05: real-provider stdio messages (MiniMax request/response mapping, provider error categories) are equally non-canonical; `speech.tts.chunk.payload.audio_ref` may resolve to a local content-addressed artifact id (`art_*`, kind `speech`) — audio bytes, hex, and provider envelopes never appear in any payload. R0.9-01: the durable `asr.request` wire kind (non-streaming artifact-backed ASR) joins `tts.request`; **dialect exception on record:** the MiMo ASR provider envelope is Chat-Completions-shaped — §1's "nothing outside model-gateway constructs Chat-Completions shapes" protects the kernel/canon from the LLM dialect, and this speech-provider dialect is confined to the MiMo worker adapter + fixtures (plus a no-audio shape echo in the gateway's pre-I/O size predictor); ASR route denial surfaces as the `progress.update` stage string `voice.asr.route-denied` — no new canonical types.* **HTTP endpoints** (`/health`, `/meta`, `/sessions`) share the gateway token via `Authorization: Bearer` (`/health` may be exempted by config for probes — default requires auth only for `/sessions`).
