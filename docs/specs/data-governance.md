...
*Duplex frame labels since M3-02: labels carried on gateway⇄worker duplex control frames are **advisory metadata only** — the gateway-side profile floor is authoritative. `clampVoiceFrameLabels` applies a one-way per-axis max: an advisory frame label can raise above the floor (e.g. `secret/local-only` honored), but a frame claiming `public/global-ok` still yields the profile floor on the emitted `speech.asr.final` and `turn.input` (test-gated in `voice.duplex-transport-v0`). No code path copies frame labels directly into event envelopes. All enforcement points of the M3-01 note above apply identically to duplex-produced turns. M3-03: identical inheritance re-asserted E2E over the loopback WebSocket path; additionally, every voice WS listener requires token auth on connect (`4401` otherwise, zero frames processed) and binds loopback only — an unauthenticated voice port would be an unlabeled ingress bypassing this section, and is therefore forbidden. M3-04: the supervised speech worker process is **controlled ingress under the gateway** — its executable and script path are repository-controlled (never user/CLI/config-supplied), its wire labels remain advisory under the same raise-only clamp, and its stdout protocol errors and bounded stderr are **untrusted diagnostics** redacted before reaching any event, ack, or log. Route clearance, MemoryGate, egress guard, TTS visibility, and replay apply unchanged to worker-backed voice. M3-05 (first real speech provider): **speech-provider egress is an enforcement point** — provider clearance is evaluated through the same parameterized clearance law as model routing (`canRouteToClearance`), before any worker/provider I/O; an under-cleared provider receives **zero connections and zero request bytes** (transport-level test-gated); TTS input is only the visible `turn.final` text after egress checks; **TTS output artifacts inherit the synthesized visible text's labels — synthesis never declassifies**; provider error payloads, trace ids, and worker stderr are untrusted diagnostics (bounded, redacted, never canonical); provider credentials are `secret://` references resolved at the gateway, delivered through one narrow child env key, never inline, never logged; proxies are disabled in code and excluded from worker environments. R0.9-01 (first real cloud ASR): **user audio egress is the strictest enforcement point yet** — effective ASR labels compose from the input artifact's labels × the voice floor × the raise-only advisory clamp, and clearance + speech egress run **before staging, before worker spawn, before any provider I/O**: a denied candidate yields worker spawn 0, staged bytes 0, provider connections 0, no `speech.asr.final`, no turn, no model call (owner-live verified with a real `secret / local-only` recording). Base64-encoded audio exists only inside the provider worker at the HTTP boundary — never in wire, JSONL, CLI, logs, or evidence. R0.9-02 (first browser surface): the web client is a **projection, not a peer** — a `surface=web-v0` socket gets a fixed op allowlist and a whitelist-projected, redacted event view; the gateway token is never persisted browser-side (all storage APIs forbidden, test-asserted) and never echoed gateway-side; artifact playback rights derive from the session's own canonical event references (404 on any ownership failure — no enumeration, no metadata/labels/paths in responses); browser-supplied bytes cross exactly one bounded endpoint where the gateway independently re-parses the closed WAV format before anything is registered. The browser never learns provider names, labels, or filesystem paths.*
