...
3. **Capability narrowing:** while untrusted content is present in the working set, high-risk tools flip to `ask` and cross-origin actions (e.g., "email this file") flip to `deny-escalate` — the model must surface the request to the user verbatim instead of acting. *Status: M5 (ROADMAP). As of M2-03 the kernel permission path passes trusted channel context and no instruction provenance, so provenance-driven `deny-escalate` is inert until wired (governance-hardening carry-in); injection suites must not imply an escalation the kernel does not perform.*
