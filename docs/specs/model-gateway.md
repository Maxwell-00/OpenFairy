...
- **Audio:** routed to the ASR path (voice spec); **documents:** parser first (text extraction), vision model only for scanned/graphical pages. *M3-01 note: the loopback voice transport (`packages/voice`, specs/voice-pipeline.md) is **not** a provider role and binds no model — it exercises the speech event protocol and trust stack deterministically. Real ASR/TTS provider roles — including binding `voice.fastpath` and the speech workers under `workers/speech/` — remain future M3 slices with their own conformance. M3-02 note: `MockSpeechDuplexWorker` (the duplex-protocol conformance worker) is likewise **not** `voice.fastpath`, not a provider role, and not a real ASR/TTS adapter — it exercises the worker-plane frame contract deterministically. M3-03: the same mock worker bridged over the local WebSocket adapter changes nothing — still no provider role, no model binding, no vendor integration. M3-04: the supervised Python scripted mock worker is likewise not `voice.fastpath`, not `speech.asr`/`speech.tts`, and not a provider; real ASR/TTS binding remains a later M3 slice and must carry cloud-speech data-clearance/audio-egress rules plus provider conformance — and must declare the minimum supported Python version (named M3-04 carry-in). M3-05: the first real speech provider (MiniMax T2A v2 TTS) is live — **speech providers have their own registry (`speech.providers`/`speech.roles`, voice-pipeline spec), separate from the model registry, but share the neutral clearance law**: `canRouteToClearance` was extracted (pure parameterization of `canRouteToModel`, zero semantic change to model routing) and both paths route through it. No speech vendor branch exists in model routing or the kernel; provider quirks live in the worker adapter + fixtures. The Python floor is settled at `>= 3.11`. `voice.fastpath` remains unbound.*
